-- Add comprehensive RLS policies for social platform
...
CREATE POLICY "Members can view group messages" ON public.group_messages
  FOR SELECT USING (
    EXISTS(SELECT 1 FROM group_chat_members gcm WHERE gcm.chat_id = group_messages.chat_id AND gcm.user_id = auth.uid())
  );
...
CREATE POLICY "Senders can update messages" ON public.group_messages
  FOR UPDATE USING (sender_id = auth.uid());

CREATE POLICY "Senders and admins can delete messages" ON public.group_messages
  FOR DELETE USING (
    sender_id = auth.uid() OR
    EXISTS(SELECT 1 FROM group_chat_members gcm WHERE gcm.chat_id = group_messages.chat_id AND gcm.user_id = auth.uid() AND gcm.role = 'admin')
  );
